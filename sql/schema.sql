...
-- =============================================================================
-- Staging table for bulk inserts before MERGE into main table
-- EXACT same structure as ChurnScoresHistory but with NO constraints/indexes
-- for fast bulk insert performance

IF OBJECT_ID('dbo.ChurnScoresStaging', 'U') IS NOT NULL
    DROP TABLE dbo.ChurnScoresStaging;
GO

CREATE TABLE dbo.ChurnScoresStaging (
    -- Primary key columns (no constraint on staging table)
    ...
    CUBS_Categories_Ever         INT             NULL,
    CUBS_Categories_Trend        DECIMAL(18,6)   NULL
    -- NO PRIMARY KEY, NO CONSTRAINTS, NO INDEXES (for fast bulk inserts)
);
GO
