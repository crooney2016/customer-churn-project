...
# Expected column count (based on DAX query output)
...
# Used when raw CSV has bracket notation (legacy format)
# New DAX query outputs clean names directly (CustomerId, SnapshotDate, etc.)
...
# These are for legacy format - new format has clean column names
...
def get_column_summary(df: pd.DataFrame) -> Dict[str, any]:
    ...
    feature_cols = [c for c in df.columns if str(c).startswith("[") and not str(c).startswith("Customers[")]
